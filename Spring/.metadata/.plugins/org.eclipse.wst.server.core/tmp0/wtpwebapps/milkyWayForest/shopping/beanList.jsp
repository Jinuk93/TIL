...
$(function(){
	$.ajax({
		url: '/milkyWayForest/shopping/getBeanList',  
		type: 'post',
		dataType: 'json',
		...
		// alert(JSON.stringify(data));.
			...
			//가져온 데이터를 뿌려준다
			$.each(data, function(index,items) { //each문
				$('<div>') .append($('<div>', {
					width: '70%', //dom 문법 => append 붙이 
					height: '300px',
					style : 'margin: 10px; cursor:pointer; overflow:hidden',
					position: 'relative'
					
				}).append($('<img>', {
					src: "/milkyWayForest/productImage/"+items.productImageName, //items.productImageName DB에서 가져온것을 덮어쓴다
					position: 'absolute',
					width: '100%',
					height: '100%',
